  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BE_API_URL}/users/chat/user_ai_agent/`,
        {
          method: "POST",
          headers: {
...
//       const accessToken = localStorage.getItem("accessToken")
//       const res = await fetch(`${process.env.NEXT_PUBLIC_BE_API_URL}/users/chat/history/?chat_id=${currentChat}`, {
//         headers: {
//           "Authorization": `Bearer ${accessToken}`,
//         },
//       })
//       const data = await res.json()
//       const formattedMessages = (data.messages as MessagePair[]).map((msg) => [
